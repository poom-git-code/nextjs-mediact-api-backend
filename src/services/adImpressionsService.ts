...
export const createAdImpressionWithCreditLog = async (
    input: {
        // ad_id: number;
    ...
    return await sequelize.transaction(async (transaction) => {
        ...
        if (ad.status === 'inactive') {
            throw new Error('This ad is inactive and cannot be impression');
        }
        ...
        // ตรวจสอบว่า user นี้ได้ดู ad นี้แล้วในวันนี้หรือยัง
        if (userId) {
            ...
            const existingImpression = await AdImpressionModel.findOne({
                where: {
            ...
            if (existingImpression) {
                // ถ้าเคยดูแล้วในวันนี้ จะไม่บันทึกซ้ำ
        ...
        const adImpression = await AdImpressionModel.create(
            {
                ad_id: ad.id,
                ad_media_id: media.id,
        ...
        // เพิ่ม CreditLog (ประเภท impression)
        const cost = new Decimal(ad.cost_per_impression ?? 0);
        if (cost.lessThan(0)) {
            throw new Error("Invalid cost per impression");
        }
        ...
        await CreditLogModel.create(
            {
                credit_id: credit.id,
                reference_id: adImpression.id,
                change: cost,
                type: 'impression',
                description: `Impression on ad ${ad.id}`,
                created_by: userId,
                updated_by: userId,
            },
            { transaction }
...
export const getAllAdImpressions = async () => {
    return await AdImpressionModel.findAll();
};

// Get ad impression by ID
export const getAdImpressionById = async (id: number) => {
    const adImpression = await AdImpressionModel.findByPk(id);
    if (!adImpression) {
        throw new Error('Ad impression not found');
    }
    return adImpression;
};

// Update ad impression by ID
// export const updateAdImpression = async (id: number, data: any) => {
//     const adImpression = await AdImpressionModel.findByPk(id);
//     if (!adImpression) {
//         throw new Error('Ad impression not found');
//     }

//     await adImpression.update(data);
//     return adImpression;
// };
